...
func connect_events() -> void:
	Events.on_monster_updated.connect(update_monster)
	Events.on_monster_added_to_battle.connect(bind_monster)
...
func bind_monster(monster: Monster, is_player_monster: bool):
...
func move_monster(avfx_instance: AVFXInstance, v2fs: Array[Vector2Float]):
	...
	tween.tween_property(sprite, "offset", Vector2.ZERO, 0.0).set_delay(avfx_instance.resource.delay)
	
	for v2f in v2fs:
		var v2_flip = v2f.v2 if your_pov else Vector2(-v2f.v2.x, v2f.v2.y)
		tween.tween_property(sprite, "offset", v2_flip, v2f.f)
...
func flash_monster(avfx_instance: AVFXInstance, v2s: Array[Vector2]):
	...
	tween.tween_property(sprite, "modulate", Color.WHITE, 0.0).set_delay(avfx_instance.resource.delay)
