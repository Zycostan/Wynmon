...
func avfx_flash_screen(avfx_instance: AVFXInstance, v2s: Array[Vector2]):
	...
	tween.tween_property(self, "modulate", Color.WHITE, 0.0).set_delay(avfx_instance.resource.delay)
...
func avfx_screenshake(avfx_instance: AVFXInstance, v3s: Array[Vector3]):
	...
	tween.tween_property(self, "position", Vector2.ZERO, 0.0).set_delay(avfx_instance.resource.delay)
