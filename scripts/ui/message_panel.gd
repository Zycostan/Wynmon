...
func _input(event: InputEvent) -> void:
	if current_message != null and current_message.choices.size() == 0 and event is InputEventMouseButton and event.button_index == MOUSE_BUTTON_LEFT and event.pressed:
		dismiss_message()
